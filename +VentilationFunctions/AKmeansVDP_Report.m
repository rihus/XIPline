...
function AKmeansVDP_Report(Ventilation, Proton, MainInput)
    %% ------------------------------------------------------------------------
    ...
    pptxFileName = ['AKmeansVDP_Report_' todayStr];
    pptxName       = fullfile(pptDir, [pptxFileName,'.pptx']);
    % pptoutputPattern = fullfile(pptDir, 'AKmeansVDP_Report_*.pptx');    
    ...
    refStr = { '-', '-', ...
        ...
        Ventilation.HealthyRef.AK.VDPULN,...
        Ventilation.HealthyRef.AK.LVV, ...
        Ventilation.HealthyRef.AK.HVV, ...
    ...
    figVDP = makeTwoRowMontage(Ventilation.AkmeansDefectmap2, 'VDP Map');
    ...
    Venthist = imread(fullfile(pptDir, char(foldername), 'Adaptive_Kmeans_Histogram.png'));
