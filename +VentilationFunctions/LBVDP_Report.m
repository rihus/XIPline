...
function LBVDP_Report(Ventilation, Proton, MainInput)
    %% ------------------------------------------------------------------------
    ...
    pptxFileName = ['LBVDP_Report_' todayStr];
    pptxName       = fullfile(pptDir, [pptxFileName,'.pptx']);
    % pptoutputPattern = fullfile(pptDir, 'LBVDP_Report_*.pptx');    
    ...
    refStr = { '-', '-', ...
        ...
        Ventilation.HealthyRef.LB.VDPULN,...
        Ventilation.HealthyRef.LB.LVV, ...
        Ventilation.HealthyRef.LB.HVV, ...
    ...
    settings = {MainInput.Scanner,MainInput.ScannerSoftware,MainInput.SequenceType,MainInput.Recon,MainInput.AnalysisCode_hash,MainInput.denoiseXe,N4bias,Ventilation.LB_Normalization,Ventilation.HealthyRef.AgeCorrected,MainInput.ImageQuality, MainInput.Note,todayStr, MainInput.Analyst};
    % Header row
    ...
    if strcmp(Ventilation.DDI2D, 'yes')
        if strcmp(Ventilation.DDIDefectMap, 'Linear Binning')
    ...
    valStr = { Ventilation.SNR_lung, Ventilation.SNR_vv, meanCV, vhi, Ventilation.LB_VDP, Ventilation.BinsPercent(2), Ventilation.BinsPercent(5)+Ventilation.BinsPercent(5), Ventilation.skewness, Ventilation.kurtosis, DDI2D,DDI3D };
    ...
    figVDP = makeTwoRowMontage(Ventilation.BinnedVentmap, 'VDP Map');
    ...
    Venthist = imread(fullfile(pptDir, char(foldername), 'LB_Ventilation_Histogram.png'));
